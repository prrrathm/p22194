...
export function BlockList({
  ...
  const flushSave = useCallback((blockId: string, text: string) => {
    ...
    if (existing) clearTimeout(existing);
    saveTimers.current.delete(blockId);
    updateBlock(blockId, { content_state: encodeContent(text) }).catch((err) => {
      console.error("Failed to save block:", err);
    });
  }, []);
  ...
  async function handleDrop(e: React.DragEvent, targetBlockId: string) {
    ...
    const insertIdx =
      dragPosition === "above" ? toTargetIdx : toTargetIdx + 1;
    ...
    const insertAfterBlockId = finalIdx === 0 ? null : tempFields[finalIdx - 1].id;
  ...
  function fieldToBlock(field: FieldArrayWithId<DocumentFormValues, "blocks", "rhfId">): BlockField & { document_id: string; content_state: string } {
    return {
      ...field,
